...
│   │       ├── New features and enhancements (such as Hosted Control Planes advancements)
...
│   │       └── Master node components and topology (including Hosted Control Planes overview)
│   └── support
│       ├── Support overview
...
│   │       └── Mirroring the OpenShift release images using oc-mirror
...
│   │       ├── Understanding OpenShift software-defined networking (SDN)
│   │       └── Choosing a network plugin provider (OVN-Kubernetes default)
...
│   │       └── Configuring egress IPs and egress firewalls
│   ├── routes
...
│           └── Configuring CSI volume snapshots, clones, and secrets store CSI driver
...
│   │       └── Forwarding cluster logs to external systems (Elasticsearch, Kafka, Splunk) via Vector
