...
│   │       ├── New features and enhancements (such as advanced cluster monitoring improvements)
...
│   │       └── Master node components and topology (Hosted Control Planes consolidation)
│   └── support
│       ├── Support overview
...
│   │       └── Mirroring the OpenShift release images using oc-mirror v2 (with lifecycle management)
...
│   │       └── Understanding the OpenShift Update Graph, update insights, and conditional updates
│   └── migrating-clusters
...
│   │       ├── Understanding OpenShift software-defined networking (OVN-Kubernetes engine features)
│   │       └── Core networking requirements and plugin strategies
│   ├── cluster-network-operators
│   │   └── Cluster Network Operator configurations
│   │       ├── Modifying Cluster Network Operator (CNO) custom resources
│   │       └── Managing proxy, egress IPs, and firewall configurations for the network
│   ├── ovn-kubernetes
│   │   └── About OVN-Kubernetes default network provider
│   │       ├── Hybrid networking and interconnectivity with OVN-Kubernetes
│   │       └── Configuring egress IPs, egress firewalls, and admin network policies
│   ├── routes
│   │   └── Configuring and managing cluster routes
│   │       ├── Creating secure routes (Edge, Passthrough, Re-encryption)
│   │       └── Customizing route timeouts, annotations, and logging
│   ├── ingress-controller
│   │   └── Configuring Ingress Controller routing
│   │       ├── Scaling, tuning, and securing the Ingress Controller
│   │       └── Sharding the Ingress Controller using namespace or route selectors
│   └── network-plugins
...
│   │       └── Managing storage classes and volume snapshots
│   ├── persistent-storage
...
│   │       └── Configuring volume resizing, expansion, and capacity tracking
│   └── container-storage-interface
│       └── CSI (Container Storage Interface) plugin overview
│           ├── Managing CSI inline volumes and persistent storage performance
...
│   │       └── Serving, routing, and eventing with Knative Services
│   ├── building-applications
│   │   └── Creating and managing applications on OpenShift Container Platform
│   │       ├── Using Source-to-Image (S2I) to build code directly
│   │       └── Configuring application deployment strategies (Rolling, Recreate, Custom)
│   ├── images
...
│   │       └── Configuring the internal cluster image registry and pull secrets
...
│   │       └── Controlling Operator lifecycles, catalogs, and subscription channels
...
│   │       └── Configuring Prometheus alerts, targets, and managing Silences
│   ├── cluster-logging
│   │   └── Deploying and managing Red Hat OpenShift Logging
│   │       ├── Deploying the OpenShift Logging and Vector operators
│   │       └── Forwarding cluster logs to external telemetry systems (Elasticsearch, Kafka, Splunk) via Vector Engine
...
│           ├── Creating VMs from operating system templates and instances
│           └── Implementing live migration and high availability for Virtual Machines across nodes
├── 10-ci-cd-and-gitops
│   ├── pipelines
│   │   └── CI/CD workflows using OpenShift Pipelines (Tekton)
│   │       ├── Installing the OpenShift Pipelines Operator
│   │       └── Creating cloud-native CD tasks, pipelines, custom resolvers, and event triggers
│   └── gitops
│       └── Declarative cluster management using OpenShift GitOps (Argo CD)
│           ├── Installing the OpenShift GitOps Operator
│           └── Deploying multi-cluster and application states from Git repositories using ApplicationSets
└── 11-api-reference
    ├── kubernetes-api
